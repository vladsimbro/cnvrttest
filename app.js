(function(){
  ...
  const chatKey = 'uuid_converter_chat';
  const chatNickKey = 'uuid_converter_chat_nick';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function readChat(){
      let items = [];
      try{ items = JSON.parse(localStorage.getItem(chatKey)) || []; }catch(e){ items = []; }
      return Array.isArray(items) ? items : [];
    }
    function writeChat(items){
      try{ localStorage.setItem(chatKey, JSON.stringify(items)); }catch(e){}
    }
    function readNick(){
      let n = '';
      try{ n = localStorage.getItem(chatNickKey) || ''; }catch(e){ n = ''; }
      return String(n || '').trim().slice(0,24);
    }
    function writeNick(n){
      try{ localStorage.setItem(chatNickKey, String(n || '').trim().slice(0,24)); }catch(e){}
    }
    ...
    function renderChat(){
      ...
      if(items.length === 0){
        chatList.textContent = 'Нет сообщений';
        return;
      }
      chatList.innerHTML = items.map(m => `
        <div class="chat-item" data-id="${escapeHtml(m.id)}">
          <div class="chat-meta">
            <div class="chat-who">
              <span class="chat-author">${escapeHtml(m.author || 'Гость')}</span>
              <span class="chat-time">${escapeHtml(formatTime(m.time))}</span>
            </div>
            <button type="button" class="ghost chat-del" data-action="chat-delete" data-id="${escapeHtml(m.id)}" aria-label="Удалить сообщение">Удалить</button>
          </div>
    ...
    function sendChat(){
      ...
      const author = (chatNick ? String(chatNick.value || '').trim().slice(0,24) : readNick()) || 'Гость';
      if(chatNick) { chatNick.value = author; writeNick(author); }
    ...
    if(chatNick){
      chatNick.value = readNick();
      chatNick.addEventListener('input', () => writeNick(chatNick.value));
    }

    if(chatBtn){
      chatBtn.addEventListener('click', () => {
        renderChat();
        openModal(chatModal, chatBtn, chatInput || closeChat);
      });
    }
    if(closeChat){
      closeChat.addEventListener('click', () => closeModal(chatModal, chatBtn));
    }
    if(chatSend){
    ...
    if(chatClearBtn){
      chatClearBtn.addEventListener('click', async () => {
        const ok = await confirmUI({
          title: 'Очистить чат',
          message: 'Удалить все сообщения чата? Это действие нельзя отменить.',
          okText: 'Очистить',
          cancelText: 'Отмена',
          danger: true
        });
    ...
    if(chatList){
      chatList.addEventListener('click', async (e) => {
        const btn = e.target.closest('button[data-action="chat-delete"]');
        if(!btn) return;
        ...
        const ok = await confirmUI({
          ...
          message: 'Удалить это сообщение из чата?',
          okText: 'Удалить',
          cancelText: 'Отмена',
          danger: true
        });
    ...
    window.addEventListener('keydown', (e) => {
      if(e.key === 'Escape'){
        ...
        if(chatModal && chatModal.classList.contains('show')) closeModal(chatModal, chatBtn);
