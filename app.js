(function(){
  const APP_VERSION = window.APP_VERSION || '1.2.7';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const treePlusHour = document.getElementById('treePlusHour');
      ...
      treeHistoryContent.innerHTML = hist.map((it, idx)=>`
        <div class="history-item" data-id="${escapeHtml(it.id)}">
          <div class="history-header" data-idx="${idx}" tabindex="0" aria-expanded="false">
            <div class="hh-left">
              <div class="hh-name">${escapeHtml(it.name || '')}</div>
...
`;
    ...
    function loadTreePrefs(){
      try{
        ...
        if(!raw) return;
        const p = JSON.parse(raw);
        if(treeAction && (p.action === 'disable' || p.action === 'enable')) treeAction.value = p.action;
      }catch(e){}
    }
    function saveTreePrefs(){
      try{
        ...
        localStorage.setItem(treePrefsKey, JSON.stringify(p));
      }catch(e){}
    }
    ...
    function buildTreePayload(){
      ...
      if(!token){
        toast('Введите access_token', 'danger');
        return null;
      }
    ...
    function initTree(){
      ...
      treePlusHour && treePlusHour.addEventListener('click', () => {
        try{
          const now = new Date();
          const msk = new Date(now.getTime() + 3*60*60*1000 + 60*60*1000);
          const y = msk.getUTCFullYear();
          const mo = String(msk.getUTCMonth()+1).padStart(2,'0');
          const d = String(msk.getUTCDate()).padStart(2,'0');
          const h = String(msk.getUTCHours()).padStart(2,'0');
          const mi = String(msk.getUTCMinutes()).padStart(2,'0');
          if(treeToDate) treeToDate.value = `${y}-${mo}-${d}`;
          if(treeToTime) treeToTime.value = `${h}:${mi}`;
          toast('По: +1 час (МСК)', 'success');
        }catch(e){}
      });
      ...
      treeBuild && treeBuild.addEventListener('click', () => {
        if(treeOutput) treeOutput.value = '';
      ...
      treeClear && treeClear.addEventListener('click', () => {
        if(treeUuids) treeUuids.value = '';
        if(treeOutput) treeOutput.value = '';
        if(treeFrom) treeFrom.value = '';
        if(treeTo) treeTo.value = '';
        toast('Очищено', 'success');
      });
