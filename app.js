...
`;
    ...
    function loadTreePrefs(){
      try{
        ...
        if(!raw) return;
        const p = JSON.parse(raw);
        if(treeToken && typeof p.token === 'string') treeToken.value = p.token;
    ...
    function saveTreePrefs(){
      try{
        const p = {
          token: treeToken ? String(treeToken.value || '') : '',
    ...
    function buildTreePayload(){
      const token = treeToken ? String(treeToken.value || '').trim() : '';
      ...
      if(action === 'enable'){
        const dp = datePartFromLocal(treeFrom ? treeFrom.value : '');
        if(!dp){
          toast('Выберите дату в поле "С"', 'danger');
          treeFrom && treeFrom.focus();
          return null;
        }
        ...
        left = mskLocalToUtcIso(treeFrom ? treeFrom.value : '');
        right = mskLocalToUtcIso(treeTo ? treeTo.value : '');
        if(!left || !right){
    ...
    function initTree(){
      ...
      const applyActionUI = () => {
        ...
        if(treeTo){
          treeTo.disabled = (a === 'enable');
          if(a === 'enable') treeTo.value = '';
        }
      };
      applyActionUI();
      ...
      treeToken && treeToken.addEventListener('input', () => saveTreePrefs());

      treeBuild && treeBuild.addEventListener('click', () => {
        const payload = buildTreePayload();
        if(!payload) return;
        if(treeOutput) treeOutput.value = formatTreeJson(payload);
