(function(){
  const APP_VERSION = window.APP_VERSION || '1.2.12';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // hotkeys (UUID tab): Ctrl/Cmd+Enter => convert
    window.addEventListener('keydown', (e) => {
      const isUuidActive = !panelTree || panelTree.style.display === 'none';
      if(isUuidActive && (e.ctrlKey || e.metaKey) && e.key === 'Enter'){
        e.preventDefault();
        uuidConvert();
      }
    });
    ...
    const treeFromDate = document.getElementById('treeFromDate');
    const treeFromTime = document.getElementById('treeFromTime');
    const treeToDate = document.getElementById('treeToDate');
    const treeToTime = document.getElementById('treeToTime');
...
const treeUuids = document.getElementById('treeUuids');
    ...
    function renderTreeHistory(){
      ...
      treeHistoryContent.querySelectorAll('button[data-action]').forEach(btn=>{
        btn.addEventListener('click', async () => {
          ...
          if(action === 'copy'){
            try{
              await navigator.clipboard.writeText(hist2[idx].output || '');
      ...
      historyContent.addEventListener('click', async (ev) => {
        ...
        if(btn){
          ...
          if(action === 'rename'){
            ...
            const actionsEl = header.querySelector('.hh-actions');
            const leftEl = header.querySelector('.hh-left');
            if(!leftEl || !actionsEl) { header.dataset.editing = ''; return; }

            const currentName = hist2[idx].name || '';

            const edit = document.createElement('div');
            edit.className = 'hh-edit';
            edit.innerHTML = `
              <input class="hh-input" type="text" maxlength="64" value="${escapeHtml(currentName)}" aria-label="Название записи" />
              <button type="button" class="hh-btn ghost small" data-save="1">Сохранить</button>
              <button type="button" class="hh-btn ghost small" data-cancel="1">Отмена</button>
            `;

            const nameEl = leftEl.querySelector('[data-role="name"]') || leftEl.querySelector('.hh-name');
            if(nameEl) nameEl.style.display = 'none';
            const timeEl = leftEl.querySelector('.hh-time');
            if(timeEl) timeEl.style.display = 'none';

            // hide actions temporarily
            Array.from(actionsEl.children).forEach(ch => { (ch instanceof HTMLElement) && (ch.style.display='none'); });
            header.appendChild(edit);

            const inputEl = edit.querySelector('input');
            const saveBtn = edit.querySelector('[data-save]');
            const cancelBtn = edit.querySelector('[data-cancel]');
            ...
            const cleanup = () => {
              ...
              Array.from(actionsEl.children).forEach(ch => { (ch instanceof HTMLElement) && (ch.style.display=''); });
              header.dataset.editing = '';
            };

            const save = () => {
              const nv = String(inputEl ? inputEl.value : '').trim();
              hist2[idx].name = nv || hist2[idx].name || 'Запись';
              writeTreeHistory(hist2);
              renderTreeHistory();
              toast('Сохранено', 'success');
            };
    ...
    function saveTreePrefs(){
      try{
        const prefs = {
          ...
          fromDate: treeFromDate ? String(treeFromDate.value || '') : '',
          fromTime: treeFromTime ? String(treeFromTime.value || '') : '',
          toDate: treeToDate ? String(treeToDate.value || '') : '',
          toTime: treeToTime ? String(treeToTime.value || '') : '',
          uuids: treeUuids ? String(treeUuids.value || '') : ''
        };
    ...
    function loadTreePrefs(){
      ...
      try{
        ...
        if(treeFromDate && p.fromDate) treeFromDate.value = p.fromDate;
        if(treeFromTime && p.fromTime) treeFromTime.value = p.fromTime;
        if(treeToDate && p.toDate) treeToDate.value = p.toDate;
        if(treeToTime && p.toTime) treeToTime.value = p.toTime;
        if(treeUuids && p.uuids) treeUuids.value = p.uuids;
      }catch(e){}
    }
...
function buildTreePayload(){
      ...
      if(action === 'enable'){
        const dp = (function(){
          const d = String(treeFromDate ? treeFromDate.value : '').trim();
          const m = d.match(/^(\d{4})-(\d{2})-(\d{2})$/);
          if(!m) return null;
          return { y: parseInt(m[1],10), mo: parseInt(m[2],10), d: parseInt(m[3],10) };
        })();
        if(!dp){
          toast('Выберите дату в поле "С"', 'danger');
          treeFromDate && treeFromDate.focus();
          return null;
        }
        ...
        const fromDT = combineDateTime(treeFromDate ? treeFromDate.value : '', treeFromTime ? treeFromTime.value : '');
        const toDT = combineDateTime(treeToDate ? treeToDate.value : '', treeToTime ? treeToTime.value : '');
        left = mskLocalToUtcIso(fromDT);
        right = mskLocalToUtcIso(toDT);
        if(!left || !right){
      ...
      const payload = {
        access_token: token,
        store_uuids: uuids,
        disable_eta: isDisable,
        disable_slot: isDisable,
        left_border: left,
        right_border: right
      };
    ...
    function initTree(){
      ...
      try{
        ...
        if(treeFromDate && !treeFromDate.value) treeFromDate.value = dateStr;
        if(treeFromTime && !treeFromTime.value) treeFromTime.value = `${h}:${mi}`;
        ...
        const dateStr2 = `${y2}-${mo2}-${d2}`;
        if(treeToDate && !treeToDate.value) treeToDate.value = dateStr2;
        if(treeToTime && !treeToTime.value) treeToTime.value = `${h2}:${mi2}`;
      }catch(e){}
      ...
      treeNow && treeNow.addEventListener('click', () => {
        try{
          ...
          const h = String(msk.getUTCHours()).padStart(2,'0');
          const mi = String(msk.getUTCMinutes()).padStart(2,'0');
          if(treeFromDate) treeFromDate.value = `${y}-${mo}-${d}`;
          if(treeFromTime) treeFromTime.value = `${h}:${mi}`;
          toast('С: сейчас (МСК)', 'success');
        }catch(e){}
      });
      const applyActionUI = () => {
        const a = String(treeAction.value || 'disable');
        if(treeToWrap){
          treeToWrap.style.display = (a === 'enable') ? 'none' : '';
        }
        if(treeFromTime) treeFromTime.disabled = (a === 'enable');
        if(treeToDate) treeToDate.disabled = (a === 'enable');
        if(treeToTime) treeToTime.disabled = (a === 'enable');
        if(a === 'enable'){
          if(treeToDate) treeToDate.value = '';
          if(treeToTime) treeToTime.value = '';
        }
      };
      applyActionUI();

      // autosave prefs
      treeFromDate && treeFromDate.addEventListener('change', saveTreePrefs);
      treeFromTime && treeFromTime.addEventListener('change', saveTreePrefs);
      treeToDate && treeToDate.addEventListener('change', saveTreePrefs);
      treeToTime && treeToTime.addEventListener('change', saveTreePrefs);
      ...
      treeClear && treeClear.addEventListener('click', () => {
        ...
        if(treeFromDate) treeFromDate.value = '';
        if(treeFromTime) treeFromTime.value = '';
        if(treeToDate) treeToDate.value = '';
        if(treeToTime) treeToTime.value = '';
        toast('Очищено', 'success');
      });
      ...
      treeHistoryBtn && treeHistoryBtn.addEventListener('click', () => {
        updateTreeHistoryBadge();
        renderTreeHistory();
        if(!treeHistoryModal) return;
        treeHistoryModal.style.display = 'flex';
        requestAnimationFrame(() => {
          treeHistoryModal.classList.add('show');
          treeHistoryModal.setAttribute('aria-hidden','false');
          (closeTreeHistory || clearTreeHistoryBtn)?.focus();
        });
      });

      closeTreeHistory && closeTreeHistory.addEventListener('click', () => {
        if(!treeHistoryModal) return;
        treeHistoryModal.classList.remove('show');
        treeHistoryModal.setAttribute('aria-hidden','true');
        setTimeout(()=> treeHistoryModal.style.display='none', 220);
      });
